...
export default function AddQuestion() {
  const [form, setForm] = useState({
    subject:'math', topic:'', text:'',
    correct_text_answer:'', solution_description:''
  })
  ...
  async function submit(e) {
    ...
    setSuccess(true)
    setForm({ subject:'math', topic:'', text:'', correct_text_answer:'', solution_description:'' })
    setChoices([
      {label:'A',text:'',is_correct:false},{label:'B',text:'',is_correct:false},
      {label:'C',text:'',is_correct:false},{label:'D',text:'',is_correct:false}
    ])
    setImage(null)
      ...
      <main style={{ maxWidth:680, margin:'2rem auto', padding:'0 1rem' }}>
        ...
        <form onSubmit={submit} style={{ background:'#fff', borderRadius:12, padding:'1.5rem', boxShadow:'0 2px 10px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <textarea rows={3} style={{ ...inp, resize:'vertical' }} required value={form.text}
              onChange={e => setForm({...form,text:e.target.value})} placeholder="Type question here..." />
          </div>
          ...
          <div style={{ marginBottom:16 }}>
            <label style={lbl}>This question's answer type</label>
            <select style={inp} value={answerType} onChange={e => setAnswerType(e.target.value)}>
          ...
          {answerType === 'mcq' && (
            <div style={{ marginBottom:16 }}>
              <label style={lbl}>Answer Choices <span style={{ color:'#888', fontWeight:400 }}>(select radio = correct answer)</span></label>
              {choices.map((c,i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                  ...
                  <input style={{ ...inp, flex:1, marginBottom:0 }} value={c.text}
                    onChange={e => setChoices(choices.map((cc,j) => j===i ? {...cc,text:e.target.value} : cc))}
                    placeholder={`Option ${c.label}`} required />
                </div>
              ))}
            </div>
          )}
          ...
          {answerType === 'typed' && (
            <div style={{ marginBottom:16 }}>
              <label style={lbl}>Correct Answer <span style={{ color:'#e74c3c' }}>*</span></label>
              <input style={inp} value={form.correct_text_answer}
                onChange={e => setForm({...form, correct_text_answer:e.target.value})}
                placeholder="Type the exact correct answer here..." />
              <p style={{ fontSize:11, color:'#888', marginTop:4 }}>
                Student's answer will be compared to this. Case-insensitive exact match.
              </p>
            </div>
          )}

          {/* Solution description — for both types */}
          <div style={{ marginBottom:16 }}>
            <label style={lbl}>Solution / Explanation (optional)</label>
            <textarea rows={3} style={{ ...inp, resize:'vertical' }}
              ...
              placeholder="Explain how to arrive at the correct answer — shown to students after submission if enabled..." />
          </div>

          <button type="submit" style={{ width:'100%', padding:'12px', background:'linear-gradient(135deg,#667eea,#764ba2)', color:'#fff', border:'none', borderRadius:8, fontWeight:600, fontSize:15, cursor:'pointer' }}>
            Save Question
          </button>
        </form>
      </main>
