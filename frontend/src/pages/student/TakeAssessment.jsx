...
export default function TakeAssessment() {
  ...
  useEffect(() => {
    ...
    api.post(`/assessments/${id}/start/`)
      ...
      .catch(e => {
        alert(e.response?.data?.error || 'Cannot start')
      })
  ...
  async function submitExam() {
    ...
    try {
      ...
      Object.entries(answers).forEach(([qId, ans]) => {
        ...
        if (ans.image) {
          fd.append(`image_${qId}`, ans.image)
        }
        ...
        <div style={{
          ...
          {result.has_image_upload && (
            <p style={{
              ...
              Your uploaded answers will be reviewed by your teacher.
            </p>
          )}
  ...
  const mins = timeLeft !== null
    ? String(Math.floor(timeLeft/60)).padStart(2,'0')
    : '--'

  const secs = timeLeft !== null
      ...
      <header style={{
        ...
        {timeLeft !== null && (
          <span style={{
            fontWeight:700,
            fontSize:20,
            color: timeLeft < 300 ? '#e74c3c' : '#27ae60',
        ...
        <button
          ...
          style={{
            padding:'8px 20px',
            background: submitting ? '#999' : '#e74c3c',
            color:'#fff',
            border:'none',
            borderRadius:8,
            fontWeight:600,
